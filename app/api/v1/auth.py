...
async def get_current_user(
    ...
    try:
        ...
        user_id = await db_service.get_user_from_session(session_id)
...
async def get_current_session(
    ...
    try:
        ...
        token = sanitize_string(credentials.credentials)
        session_id = verify_token(token)
        print("session_id 3:", session_id)
        # session_id = "b36c4029-b037-4626-a47f-36b8fd5a53ff"
        if session_id is None:
...
async def login(
    ...
    try:
        ...
        try:
            ...
            logger.error("SSSSS session_not_found", error=str(e), exc_info=True)
        # token = create_access_token(str(user.id))
        # return TokenResponse(access_token=token.access_token, token_type="bearer", expires_at=token.expires_at)
    except ValueError as ve:
...
async def update_session_name(
    ...
    """
    try:
        logger.info(f"sesion id 1:{session_id}")
        # Sanitize inputs
        sanitized_session_id = sanitize_string(session_id)
        logger.info(f"sesion id 2:{sanitized_session_id}")
