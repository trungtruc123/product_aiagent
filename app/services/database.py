...
class DatabaseService:
    ...
    async def get_user_from_session(self, session_id: str) -> int:
        with Session(self.engine) as session:
            chat_session = session.get(ChatSession, session_id)
            if not chat_session:
                logger.error("Don't found user from session", session_id=session_id)
    ...
    async def get_session(self, session_id: str) -> Optional[ChatSession]:
        ...
        """
        print("Session to find in database:", session_id)
        with Session(self.engine) as session:
            chat_session = session.get(ChatSession, session_id)
